...
public class FilterToElasticTest {
	...
	public void testNot() {
		...
		// NotFilterBuilder expected =
		// FilterBuilders.notFilter(FilterBuilders.idsFilter().addIds("id"));
...
//		QueryFilterBuilder expected = FilterBuilders
//				.queryFilter(QueryBuilders.queryString("hello").defaultField("analyzed"));
...
//				FilterBuilders.geoShapeFilter("geom", shape, ShapeRelation.INTERSECTS));
	...
	public void testGeoPointBboxFilter() {
		...
		GeoBoundingBoxQueryBuilder expected = QueryBuilders.geoBoundingBoxQuery("geo_point").topLeft(1, 0)
				.bottomRight(0, 1);
//		GeoBoundingBoxFilterBuilder expected = FilterBuilders.geoBoundingBoxFilter("geo_point").topLeft(1, 0)
//				.bottomRight(0, 1);
...
//		GeoPolygonFilterBuilder expected = FilterBuilders.geoPolygonFilter("geo_point").addPoint(0, 0).addPoint(1, 0)
//				.addPoint(1, 1).addPoint(0, 1).addPoint(0, 0);
	...
	public void testDWithinFilter() throws CQLException {
		...
		GeoDistanceQueryBuilder expected = QueryBuilders.geoDistanceQuery("geo_point").lat(1).lon(0).distance(1.,
				DistanceUnit.METERS);
//		GeoDistanceFilterBuilder expected = FilterBuilders.geoDistanceFilter("geo_point").lat(1).lon(0).distance(1.,
//				DistanceUnit.METERS);
	...
	public void testDWithinPolygonFilter() throws CQLException {
		...
		GeoDistanceQueryBuilder expected = QueryBuilders.geoDistanceQuery("geo_point").lat(0.5).lon(0.5).distance(1.,
				DistanceUnit.METERS);
//		GeoDistanceFilterBuilder expected = FilterBuilders.geoDistanceFilter("geo_point").lat(0.5).lon(0.5).distance(1.,
//				DistanceUnit.METERS);
...
//		NotFilterBuilder expected = FilterBuilders.notFilter(
//				FilterBuilders.geoDistanceFilter("geo_point").lat(1).lon(0).distance(1., DistanceUnit.METERS));
...
//		RangeFilterBuilder expected = FilterBuilders.rangeFilter("dateAttr").gt("1970-07-19T01:02:03.456Z")
//				.lt("1970-07-19T07:08:09.101Z");
